...
//check the . forto make sure it is a decimal number
     ...
     */
    public static double evaluateExpression(String express) {
 ...
 */
    private static void compute(Stack<Double> values, String op) {
        if (op.equals("sin") || op.equals("cos") || op.equals("tan") ||
            ...
            double a = values.pop();
            // If your input is in radians, do not convert to radians again
            switch (op) {
                case "sin":
                    values.push(Math.sin(a)); // a is already in radians
                    break;
                case "cos":
                    values.push(Math.cos(a)); // a is already in radians
                    break;
                case "tan":
                    values.push(Math.tan(a)); // a is already in radians
                    break;
                case "cot":
                    values.push(1 / Math.tan(a)); // cot(x) = 1 / tan(x)
                    break;
                case "ln":
                    if (a <= 0) {
            ...
            switch (op) {
                ...
                case "^":
                    ...
                    if (Double.isInfinite(result)|| Double.isNaN(result)) {
                    	throw new ArithmeticException("Result: Error or buffer overflow (numeric) ");
                    }
                    values.push(result);
                    break;
            }
